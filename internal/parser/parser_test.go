...
func (suite *ParserTestSuite) TestSizeOf() {
	...
	_, err := p.ParseString("main.c", "sizeof(i32)")
	suite.NoError(err)
	_, err = p.ParseString("main.c", "sizeof 2")
	suite.NoError(err)
}
...
func (suite *ParserTestSuite) TestNull() {
	suite.T().Run("PrimaryExpr NULL", func(t *testing.T) {
		p := parser.BuildParser[parser.PrimaryExpr]()

		_, err := p.ParseString("main.c", `NULL`)
		suite.NoError(err)
	})

	suite.T().Run("NULL", func(t *testing.T) {
		p := parser.BuildParser[parser.DeclStmt]()

		_, err := p.ParseString("main.c", `i64 *a=NULL;`)
		suite.NoError(err)
	})

	suite.T().Run("Module NULL", func(t *testing.T) {
		p := parser.BuildParser[parser.Module]()
...
func (suite *ParserTestSuite) TestWhile() {
	p := parser.BuildParser[parser.WhileStmt]()
...
func (suite *ParserTestSuite) TestContinue() {

	suite.T().Run("continue", func(t *testing.T) {
		p := parser.BuildParser[parser.ContinueStmt]()
		_, err := p.ParseString("main.c", `
        continue;`)
		suite.NoError(err)
	})
	suite.T().Run("continue", func(t *testing.T) {
		p := parser.BuildParser[parser.IfStmt]()
...
func (suite *ParserTestSuite) TestBreak() {
	suite.T().Run("break", func(t *testing.T) {
		p := parser.BuildParser[parser.BreakStmt]()
		_, err := p.ParseString("main.c", `
        break;`)
		suite.NoError(err)
	})
...
func (suite *ParserTestSuite) TestFor() {
	p := parser.BuildParser[parser.ForStmt]()
